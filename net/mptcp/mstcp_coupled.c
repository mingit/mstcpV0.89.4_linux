...
{
	struct mstcp_ccc *mstcp_ccc = inet_csk_ca(meta_sk);
	return mstcp_ccc->alpha;
}

static inline void mptcp_set_alpha(struct sock *meta_sk, u64 alpha)
{
	struct mstcp_ccc *mstcp-ccc = inet_csk_ca(meta_sk);
	mstcp-ccc->alpha = alpha;
}
